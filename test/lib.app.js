  ...
  it('should check the developer legal information from the "About the developer" section', () => {
    return gplay.app({ appId: "com.soundcloud.android" }).then((app) => {
      ...
      assert.equal(
        ...
        "Karl-Marx-Str. 101, 12043 Berlin, Germany",
      );
  ...
  it("should fetch PriceText for paid apps properly", () => {
    return gplay
      ...
      .then((app) => {
        ...
        assert.include(app.priceText, "₹");
        // priceText may include locale-specific formatting (e.g., ₹3,250.00)
        // so compare the raw numeric value after removing non-numeric chars
        const priceNumeric = app.priceText.replace(/[^\d.]/g, "");
        assert.equal(priceNumeric, app.price.toFixed(2));
